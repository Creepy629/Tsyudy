...
@export var dialog_cooldown: float = 2.0
...
const CS_GAMETAGS: Array[String] = [
	"phoon", "mundo", "pashaBiceps", "snax", "JW", "parzival",
	"dev1ce", "dupreeh", "magisk", "gla1ve", "Xyp9x", "coldzera",
	"FalleN", "fer", "TACO", "Stewie2K", "autimatic", "tarik", "twistzz"
]
...
const TEAM: String = "CT"
const ENEMY_TEAM: String = "TT"
const SHOT_STREAM: AudioStream = preload("res://Audio/Ciudad/Counter/ak47.wav")
...
# ── Skin fija CT ───────────────────────────────────────────────────────────
const SKIN_TEX = preload("res://Texturas/Personajes/CTsheet.png")
const SKIN_HEIGHT: float = 2.0
const SKIN_SCALE: Vector3 = Vector3(4.836, 4.836, 4.836)
const SKIN_POS_Y: float = -0.45

# ── Estados ────────────────────────────────────────────────────────────────
enum State {WALKING, WAITING, DEFUSING, COMBAT, DEAD_FROZEN}
...
var _assigned_site: BombSite = null
...
func _ready() -> void:
	...
	shot_player.stream = SHOT_STREAM
...
func _setup_gametag() -> void:
	...
	gametag_label.modulate = Color(0.36, 0.68, 0.95) # Azul claro CT
...
func setup_navigation() -> void:
	...
	if nav_region == null or not nav_region.get_navigation_map().is_valid():
		push_warning("NPCCT %s: sin NavigationRegion3D válida." % name)
		return
...
func choose_next_objective() -> void:
	...
	# Si la ronda terminó, seguir patrullando el sitio asignado
	if _is_round_over():
		if _assigned_site != null and is_instance_valid(_assigned_site):
			var patrol_offset := Vector3(randf_range(-8.0, 8.0), 0, randf_range(-8.0, 8.0))
			agent.target_position = _assigned_site.global_position + patrol_offset
	...
	# Si la C4 está plantada en algún Site (A o B), correr de inmediato a ese site
	if BombSite.active_planted_site != null and is_instance_valid(BombSite.active_planted_site):
		...
		if site_state == BombSite.BombState.PLANTED or site_state == BombSite.BombState.DEFUSING:
			agent.target_position = BombSite.active_planted_site.global_position
			current_state = State.WALKING
			destination_timer = 0.0
	...
	# Si no hay C4 plantada, quedarse en el sitio asignado
	if _assigned_site != null and is_instance_valid(_assigned_site):
		var patrol_offset := Vector3(randf_range(-8.0, 8.0), 0, randf_range(-8.0, 8.0))
		agent.target_position = _assigned_site.global_position + patrol_offset
		current_state = State.WALKING
	...
	# Fallback: patrullar aleatoriamente
	set_random_destination()
...
func _process_waiting(delta: float) -> void:
	...
	if BombSite.active_planted_site != null:
		choose_next_objective()
		return
...
func _process_defusing(_delta: float) -> void:
	velocity.x = 0.0
	velocity.z = 0.0
	if audio.playing:
		audio.stop()
		
	if BombSite.active_planted_site == null or BombSite.active_planted_site.current_state != BombSite.BombState.DEFUSING:
		choose_next_objective()
		return
...
func _process_combat(delta: float) -> void:
	...
	# Si la bomba está plantada y está cerca del site (< 30m), le corre
	# recio a desactivarla.
	if BombSite.active_planted_site != null and global_position.distance_to(BombSite.active_planted_site.global_position) < 30.0:
		var site_dir := (BombSite.active_planted_site.global_position - global_position)
		site_dir.y = 0.0
...
func _process_walking(delta: float) -> void:
	...
	# Si la C4 está plantada, verificar si llegamos al site activo para desactivar
	if BombSite.active_planted_site != null and is_instance_valid(BombSite.active_planted_site):
		if BombSite.active_planted_site.is_npc_in_site(self):
			if BombSite.active_planted_site.start_defusing(self):
				current_state = State.DEFUSING
				return
...
func _assign_initial_site() -> void:
	var sites: Array = get_tree().get_nodes_in_group("BombSite")
	if sites.is_empty():
		return

	# Contar cuántos CT ya están asignados a cada sitio
	var site_a_count: int = 0
	var site_b_count: int = 0

	for ct in get_tree().get_nodes_in_group(TEAM):
		if ct is NPCCT and ct != self:
			var assigned: BombSite = ct.get("_assigned_site") as BombSite
			if assigned != null:
				if assigned.name.contains("A") or assigned.name.to_lower().contains("a"):
					site_a_count += 1
				else:
					site_b_count += 1

	# Asignar al sitio con menos CTs
	var chosen_site: BombSite = null
	for s in sites:
		if s is BombSite:
			var site: BombSite = s as BombSite
			if site.name.contains("A") or site.name.to_lower().contains("a"):
				if site_a_count <= site_b_count:
					chosen_site = site
					break
			else:
				if site_b_count < site_a_count:
					chosen_site = site
					break

	if chosen_site == null and not sites.is_empty():
		chosen_site = sites[0] as BombSite

	_assigned_site = chosen_site
