...
static func throw_grenade(from_pos: Vector3, throw_dir: Vector3, g_type: GrenadeType, team: String, tree: SceneTree) -> Grenade:
	...
	sphereMesh.radius = 0.15
	sphereMesh.height = 0.3
...
func _spawn_visual_blast(color: Color, radius: float) -> void:
	var flashMesh := MeshInstance3D.new()
	...
	flashMesh.material_override = mat

	get_tree().current_scene.add_child(flashMesh)
	flashMesh.global_position = global_position

	var tw := flashMesh.create_tween()
	tw.tween_property(mat, "albedo_color:a", 0.0, 0.25)
	tw.tween_callback(flashMesh.queue_free)
