...
@onready var look: MeshInstance3D = get_parent().get_node_or_null("CollisionShape3D/MeshInstance3D")
var anim_frame := 0
var frame_timer := 0.0
@export var anim_speed := 15.0

# Esto también me da culo de quitar
...
func check_if_moving() -> bool:
	...
	if parent is CharacterBody3D:
		return parent.velocity.length() > 1
	return false
